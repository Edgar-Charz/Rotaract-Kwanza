...
require_once __DIR__ . '/classes/LeadershipTermPhoto.php';
...
$termPhotoModel = new LeadershipTermPhoto($conn);
$photosByTerm = $termPhotoModel->getByTerms(array_column($termsData, 'id'));
foreach ($termsData as &$termRowRef) {
  $gallery = array_column($photosByTerm[(int) $termRowRef['id']] ?? [], 'image_path');
  $termRowRef['photos'] = $termRowRef['image_path'] ? [$termRowRef['image_path'], ...$gallery] : $gallery;
}
unset($termRowRef);
...
$page_title = site_title($conn, 'Leadership History');
$page_description = 'Explore the officers, terms, and past leaders who have shaped the Rotaract Club of Kwanza.';
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <style>
    ...
    .history-term-marker {
      ...
      box-shadow: 0 0 0 6px var(--pink-50);
    }
    ...
    .history-timeline.timeline-focused::before {
      display: none;
    }

    .history-timeline.timeline-focused .history-term-marker {
      display: none;
    }
    ...
    .history-timeline.timeline-focused .history-term-meta {
      text-align: center;
      margin-bottom: 32px;
    }
          ...
          <div class="history-controls reveal reveal-delay-3">
            ...
            <div class="history-term-picker">
              <div class="history-term-picker-inner">
                <svg class="history-term-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                ...
                <select id="term-filter" class="history-term-select" aria-label="Select leadership term">
                  ...
                  <?php foreach ($termsData as $t): ?>
                    <option value="<?= e($t['term_label']) ?>" <?= $selectedTerm === $t['term_label'] ? ' selected' : '' ?>>
                      <?= e($t['term_label']) ?> Leadership Team</option>
                  <?php endforeach; ?>
                </select>
                <svg class="history-term-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                  stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <div class="history-timeline<?= $selectedTerm !== '' ? ' timeline-focused' : '' ?>" id="history-timeline">
          <?php foreach ($termsData as $termRow):
            ...
            <article class="history-term-block reveal" data-term="<?= e($label) ?>" <?= $hidden ? ' style="display:none"' : '' ?>>
              ...
              <?php endif; ?>
              <?php if ($termRow['photos']): ?>
                <div class="history-term-photo reveal">
                  <?= render_photo_slider($termRow['photos'], $label . ' leadership team') ?>
                </div>
              <?php endif; ?>
