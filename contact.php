...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!rate_limit_allow($conn, 'contact', 5, 900)) {
    ...
    if (!$full_name || !$email || !$msg) {
      ...
      try {
        ...
        $id = $cm->create($full_name, $email, $subject, $msg);

        $mailer = Mailer::fromSettings($conn);
        $adminEmail = (new SiteSettings($conn))->get('contact_email', '');
        if ($adminEmail) {
          $mailer->contactNotification($adminEmail, $full_name, $email, $subject, $msg);
        }
        $sent = $mailer->contactConfirmation($email, $full_name);
  ...
  // the response page re-submits the form. Old input rides along in the
...
$page_description = $contact_intro;
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div>
          ...
          <div class="contact-info reveal reveal-delay-3">
            ...
            <div class="contact-item">
              <div class="contact-item-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="var(--pink-700)" stroke-width="2" stroke-linecap="round">
                  <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 8.63a19.79 19.79 0 01-3.07-8.67A2 2 0 012.18 0h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 14.92z" />
                </svg>
              </div>
              <div>
                <h4>Call Us</h4>
          ...
          <form action="" method="POST">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" name="full_name" value="<?= e($old['full_name'] ?? '') ?>" placeholder="Your name" required>
              </div>
              <div class="form-group">
                <label>Email</label>
            ...
            <div class="form-group">
              <label>Subject</label>
              <input type="text" name="subject" value="<?= e($old['subject'] ?? '') ?>" placeholder="How can we help?">
            </div>
            <div class="form-group">
              <label>Message</label>
