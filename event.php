...
$going      = ($event && $event['status'] === 'upcoming') ? $rsvp_obj->getAttendanceSummary($id)['total'] : 0;
...
$page_title = $event ? site_title($conn, $event['title']) : site_title($conn, 'Event Not Found');
$page_description = $event
  ? mb_strimwidth(trim(strip_tags($event['description'] ?? '')), 0, 160, '…')
  : 'This event may have been removed or the link is incorrect.';
if ($event && $event['image_path']) $page_image = img_url($event['image_path']);
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <style>
    ...
    .ev-hero-fallback.gold {
      background: linear-gradient(135deg, #D4882A, #8a5312);
    }

    .ev-hero-fallback.rose {
      background: linear-gradient(135deg, #e88ab5, var(--pink-800));
    }
          ...
          <div class="ev-cta-card">
            ...
            <?php if ($event['instagram_url'] || $event['tiktok_url'] || ($event['x_url'] ?? '')): ?>
              <div class="ev-socials ev-socials--center">
                ...
                <?php endif; ?>
                <?php if ($event['x_url'] ?? ''): ?>
                  <a href="<?= e($event['x_url']) ?>" target="_blank" rel="noopener" class="ev-social-btn">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
