...
<div class="card-header card-header--standalone">
  <div>
    ...
    <p class="text-muted fs-12-5 mt-4px">Shown as a slider at the top of the public "Meet the Team" page.</p>
  </div>
        ...
        <div class="gallery-item-info">
          <?php if (has_role('editor')): ?>
            <div class="gallery-item-actions">
              <form id="del-tmp-<?= $p['id'] ?>" method="POST" class="d-inline">
                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                <input type="hidden" name="action" value="delete">
                <input type="hidden" name="id" value="<?= $p['id'] ?>">
              </form>
              <button class="btn btn-icon btn-sm btn-danger" title="Delete" aria-label="Delete" onclick="confirmDelete('del-tmp-<?= $p['id'] ?>')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <form method="POST" enctype="multipart/form-data">
      <div class="modal-body">
        ...
        <div class="form-group mb-2">
          ...
          <input type="file" id="tmp_images" name="images[]" accept="image/*" multiple class="hidden" onchange="document.getElementById('tmp_count').textContent = this.files.length + ' file(s) selected';">
          <p id="tmp_count" class="text-muted fs-12 mt-6px"></p>
        </div>
      </div>
