          ...
          <tr>
            ...
            <td>
              <div class="table-actions">
                <?php if (has_role('editor')): ?>
                  <button class="btn btn-icon btn-sm btn-info" title="Edit" aria-label="Edit" onclick="openEditModal(<?= h(json_encode($c)) ?>)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  ...
                  <button class="btn btn-icon btn-sm btn-danger" title="Delete" aria-label="Delete" onclick="confirmDelete('del-c-<?= $c['id'] ?>')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="modal-body">
        ...
        <div class="form-row">
          ...
          <div class="form-group form-group-row pt-22px">
            <input type="checkbox" name="is_active" id="ac_active" checked class="w-auto">
            <label for="ac_active" class="fw-normal">Available when tagging</label>
          </div>
        </div>
      </div>
    ...
    <form method="POST">
      <div class="modal-body">
        ...
        <div class="form-row">
          ...
          <div class="form-group form-group-row pt-22px">
            <input type="checkbox" name="is_active" id="ec_active" class="w-auto">
            <label for="ec_active" class="fw-normal">Available when tagging</label>
          </div>
        </div>
      </div>
      <div class="modal-footer">
...
<script>
  $(document).ready(function() {
    $('#dt-categories').DataTable({
      ...
      language: {
        emptyTable: 'No <?= strtolower($type_labels[$type]) ?> categories yet.'
      },
      columnDefs: [{
        orderable: false,
        targets: 3
