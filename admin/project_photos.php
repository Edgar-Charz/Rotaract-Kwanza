...
$project    = (new Project($conn))->findById($project_id);
...
$page_title = 'Photos — ' . $project['title'];
$photo_obj  = new ProjectPhoto($conn);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        if ($photo && (int)$photo['project_id'] === $project_id) {
    ...
    if ($action === 'reorder') {
        ...
        foreach ($ids as $i => $pid) {
            ...
            if (!$photo || (int) $photo['project_id'] !== $project_id) continue;
            $photo_obj->updateOrder($pid, $i);
        }
        flash('success', 'Photo order updated.');
    }

    header('Location: ' . ADMIN_URL . '/project_photos.php?project=' . $project_id);
    exit;
}

$photos = $photo_obj->getByProject($project_id);
...
<div class="card-header" style="background:#fff;border-radius:10px;padding:14px 20px;margin-bottom:20px;box-shadow:var(--shadow);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
  <div>
    <a href="projects.php" class="btn btn-sm btn-secondary" style="margin-bottom:6px;display:inline-block">&larr; Back to Projects</a>
    <div class="card-title"><?= h($project['title']) ?> — <?= count($photos) ?> Photo<?= count($photos) !== 1 ? 's' : '' ?></div>
  </div>
...
<div class="gallery-admin-grid">
  ...
  <div class="gallery-item-admin">
    <img src="<?= h($p['image_path']) ?>" alt="<?= h($p['caption'] ?: 'Project photo') ?>">
    <div class="gallery-item-info">
      <?php if ($p['caption']): ?><div class="item-title" style="font-size:12px"><?= h($p['caption']) ?></div><?php endif; ?>
      <?php if (has_role('editor')): ?>
      <div class="gallery-item-actions">
        <button class="btn btn-icon btn-sm btn-info" title="Edit Caption" aria-label="Edit Caption" onclick="openCaptionModal(<?= (int) $p['id'] ?>, <?= h(json_encode($p['caption'] ?? '')) ?>)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 3a2.85 2.86 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg></button>
        <form id="del-pp-<?= $p['id'] ?>" method="POST" style="display:inline">
          <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
          <input type="hidden" name="action" value="delete">
          <input type="hidden" name="id" value="<?= $p['id'] ?>">
        </form>
        <button class="btn btn-icon btn-sm btn-danger" title="Delete" aria-label="Delete" onclick="confirmDelete('del-pp-<?= $p['id'] ?>')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>
      </div>
      <?php endif; ?>
    </div>
  </div>
...
<div class="empty-state">
  ...
  <p>No photos yet for this project. Upload some for visitors to see on the public project page.</p>
</div>
<?php endif; ?>
...
<div class="modal fade" id="caption-modal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-content" style="max-width:400px">
    ...
    <form method="POST">
      <div class="modal-body">
        ...
        <div class="form-group">
          <label>Caption <span class="text-muted" style="font-weight:400">(shown in the public photo lightbox)</span></label>
...
<div class="modal fade" id="reorder-modal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-content" style="max-width:480px">
    ...
    <div class="modal-body">
      <p class="text-muted" style="font-size:12.5px;margin-bottom:12px">Drag to reorder. This sets the order photos appear in on the public project page.</p>
...
<script>
var allProjectPhotos = <?= json_encode(array_map(fn($p) => ['id' => $p['id'], 'image_path' => $p['image_path'], 'caption' => $p['caption'] ?? ''], $photos)) ?>;

function openReorderModal() {
  const list = document.getElementById('reorder-list');
  list.innerHTML = '';
  allProjectPhotos.forEach(function (p) {
    ...
    <form method="POST" enctype="multipart/form-data">
      <div class="modal-body">
        ...
        <div class="form-group mb-2">
          <label>Photos *</label>
          <label class="upload-area" for="pp_images">
          ...
          <input type="file" id="pp_images" name="images[]" accept="image/*" multiple style="display:none" onchange="renderCaptionInputs(this)">
          <p id="pp_count" class="text-muted" style="font-size:12px;margin-top:6px"></p>
          <div id="pp_caption_list" style="margin-top:6px"></div>
        </div>
      </div>
      <div class="modal-footer">
...
function renderCaptionInputs(input) {
  document.getElementById('pp_count').textContent = input.files.length
    ? input.files.length + ' file(s) selected'
    : '';
  const list = document.getElementById('pp_caption_list');
  list.innerHTML = '';
