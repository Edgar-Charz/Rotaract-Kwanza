...
<div class="split-layout rsvps-grid">

  <div class="card">
    <div class="card-header"><span class="card-title">Events</span></div>
    <div class="event-filter-list">
      <a href="?" class="event-filter-link <?= !$event_id ? 'active' : '' ?>">All RSVPs</a>
      <?php foreach ($events as $ev): $c = $counts[$ev['id']] ?? null; ?>
        <a href="?event=<?= $ev['id'] ?>"
          class="event-filter-link--row <?= $event_id === $ev['id'] ? 'active' : '' ?>">
          <span class="truncate"><?= h($ev['title']) ?></span>
          <?php if ($c): ?>
            <span class="event-filter-count"><?= $c['n'] ?></span>
          <?php endif; ?>
        </a>
      <?php endforeach; ?>
    </div>
  ...
  <div>
    <?php if ($event_id && $attendance): ?>
      <div class="stats-grid mb-2" style="grid-template-columns:repeat(<?= $spots_left !== null ? 4 : 3 ?>,1fr)">
        <div class="stat-card">
          <div class="stat-icon pink"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="stat-card">
          ...
          <div>
            ...
            <div class="stat-value"><?= (int)$attendance['attended'] ?></div>
          </div>
        </div>
        <div class="stat-card">
        ...
        <?php if ($spots_left !== null): ?>
          <div class="stat-card">
            <div class="stat-icon <?= $spots_left <= 0 ? 'red' : 'blue' ?>"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <tbody>
            <?php foreach ($rsvps as $r): ?>
              <tr>
                ...
                <?php if (!$event_id): ?><td class="text-muted fs-12"><?= h($r['event_title']) ?></td><?php endif; ?>
                <td class="fw-bold"><?= h($r['name']) ?></td>
                <td><?= h($r['email']) ?></td>
                <td class="text-muted"><?= h($r['phone'] ?? '—') ?></td>
                <td><?= $r['guests'] ?></td>
                <td class="text-muted text-clamp-160" title="<?= h($r['guest_names'] ?? '') ?>"><?= h($r['guest_names'] ?? '') ?: '—' ?></td>
                <td class="text-muted text-clamp-180"><?= h($r['notes'] ?? '—') ?></td>
                <?php if ($event_id): ?>
                  <td>
                ...
                <td>
                  <div class="table-actions">
                    <?php if (has_role('editor')): ?>
                      <button class="btn btn-icon btn-sm btn-info" title="Edit" aria-label="Edit" onclick="openEditRsvpModal(<?= h(json_encode($r)) ?>)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="modal-dialog modal-content modal-460">
      ...
      <form method="POST">
        <div class="modal-body">
          ...
          <?php if ($spots_left !== null): ?>
            <p class="text-muted fs-12-5 mb-12px"><?= $spots_left ?> spot<?= $spots_left === 1 ? '' : 's' ?> left for this event.</p>
          <?php endif; ?>
          <div class="form-row">
          ...
          <div class="form-group"><label>Guest Names <span class="text-muted fw-normal">(if bringing others)</span></label><input type="text" name="guest_names" placeholder="e.g. Jane Doe, John Doe"></div>
          <div class="form-group"><label>Notes</label><textarea name="notes" class="mh-70"></textarea></div>
        </div>
    ...
    <div class="modal-dialog modal-content modal-460">
      ...
      <form method="POST">
        <div class="modal-body">
          ...
          <div class="form-group"><label>Guest Names <span class="text-muted fw-normal">(if bringing others)</span></label><input type="text" name="guest_names" id="er_guest_names" placeholder="e.g. Jane Doe, John Doe"></div>
          <div class="form-group"><label>Notes</label><textarea name="notes" id="er_notes" class="mh-70"></textarea></div>
        </div>
        <div class="modal-footer">
