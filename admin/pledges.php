...
<div class="stats-row stats-row-grid">
  <div class="card">
    <div class="card-body">
      <div class="text-muted fs-12">Total Pledges</div>
      <div class="stat-value-lg"><?= $total_count ?></div>
    </div>
  </div>
  <div class="card">
    <div class="card-body">
      <div class="text-muted fs-12">Unverified</div>
      <div class="stat-value-lg"><?= $unverified_count ?></div>
    </div>
  </div>
  <div class="card">
    <div class="card-body">
      <div class="text-muted fs-12">Confirmed</div>
      <div class="stat-value-lg"><?= $confirmed_count ?></div>
    </div>
  </div>
  <div class="card">
    <div class="card-body">
      <div class="text-muted fs-12">Confirmed Amount</div>
      <div class="stat-value-lg"><?= number_format($confirmed_amount, 2) ?></div>
    </div>
  </div>
</div>
      ...
      <tbody>
        ...
        ?>
          <tr>
            ...
            <td>
              <div class="fs-12-5"><?= h($p['email']) ?></div>
              <div class="text-muted fs-11-5"><?= h($p['phone'] ?: '—') ?></div>
            </td>
            <td>
              <div class="fw-bold fs-12-5"><?= h($p['method_label'] ?: ($p['method'] === 'mobile_money' ? 'Mobile Money' : 'Bank Transfer')) ?></div>
              <div class="text-muted fs-11"><?= $p['method'] === 'mobile_money' ? 'Mobile Money' : 'Bank Transfer' ?></div>
            </td>
            <td class="text-muted fs-12-5"><?= h($p['project_title'] ?: 'General Donation') ?></td>
            <td><?= $p['amount'] !== null ? number_format((float)$p['amount'], 2) : '—' ?></td>
            <td class="text-muted text-clamp-180"><?= h($p['note'] ?: '—') ?></td>
            <td><span class="badge badge-<?= $badge ?>"><?= h(ucfirst($p['status'])) ?></span></td>
            <td class="text-muted nowrap"><?= $p['created_at'] ? date('d M Y H:i', strtotime($p['created_at'])) : '—' ?></td>
            <td>
