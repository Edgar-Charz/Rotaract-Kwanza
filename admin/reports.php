...
<div class="reports-toolbar">
  <form method="GET" class="reports-filter-form">
    <label class="reports-filter-label">
      From <input type="date" name="from" value="<?= h($from) ?>" class="reports-date-input">
    </label>
    <label class="reports-filter-label">
      To <input type="date" name="to" value="<?= h($to) ?>" class="reports-date-input">
    </label>
...
<div class="kpi-row">
  ...
  <div class="kpi-card">
    ...
    <div class="kpi-value <?= $dues_outstanding > 0 ? 'kpi-value-danger' : '' ?>"><?= number_format($dues_outstanding, 0, '.', ',') ?></div>
...
<div class="report-grid">
  ...
  <div class="chart-card">
    ...
    <div class="chart-wrap chart-wrap-280"><canvas id="statusChart" height="220" role="img" aria-label="Doughnut chart of member application status breakdown"></canvas></div>
  </div>
</div>
...
<div class="report-grid thirds">
  <div class="chart-card">
    ...
    <div class="chart-wrap chart-wrap-240"><canvas id="duesChart" height="200" role="img" aria-label="Doughnut chart of member dues records by payment status"></canvas></div>
  </div>
  <div class="chart-card">
...
<div class="report-grid thirds">
  <div class="chart-card">
    ...
    <?php if ($attend_rows): ?>
      <table class="attend-table">
        <thead>
          <tr>
            <th>Event</th>
            <th class="w-80 text-right">Attended</th>
            <th class="w-80 text-right">Registered</th>
        ...
        <tbody>
          ...
          ?>
            <tr>
              ...
              <td class="attend-count"><?= $ar['attended'] ?></td>
              <td class="attend-total"><?= $ar['total'] ?></td>
              <td>
                <div class="attend-rate"><?= $rate ?>%</div>
                <div class="attend-bar">
                  <div class="attend-fill" style="width:<?= $rate ?>%"></div>
                </div>
              </td>
            </tr>
          <?php endforeach; ?>
    ...
    <?php else: ?>
      <p class="text-muted table-empty-30-0">No attendance data yet.</p>
    <?php endif; ?>
  </div>
  <div class="chart-card">
...
<div class="chart-card mb-20px">
  ...
  <div class="impact-total">
    ...
    <span class="impact-total-label">combined impact (parsed from each project's Impact Stat)</span>
  </div>
  <?php if ($project_rows): ?>
    <table class="attend-table">
      <thead>
      ...
      <tbody>
        <?php foreach ($project_rows as $pr): ?>
          <tr>
            ...
            <td><?= $pr['impact_stat'] ? h($pr['impact_stat']) . ($pr['impact_label'] ? ' ' . h($pr['impact_label']) : '') : '<span class="text-empty">—</span>' ?></td>
  ...
  <?php else: ?>
    <p class="text-muted table-empty-30-0">No projects yet.</p>
  <?php endif; ?>
</div>
  ...
  (function() {
    ...
    new Chart(document.getElementById('statusChart'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: [green, gold, '#e74c3c'],
          borderWidth: 2
        }]
      },
      options: {
    ...
    new Chart(document.getElementById('rsvpChart'), {
      ...
      data: {
        ...
        datasets: [{
          label: 'RSVPs',
          data: revValues,
          backgroundColor: 'rgba(212,136,42,0.85)',
          borderRadius: 5
        }]
      },
      options: {
    ...
    new Chart(document.getElementById('duesChart'), {
      ...
      data: {
        ...
        datasets: [{
          data: [<?= $dues_data['paid']['count'] ?>, <?= $dues_data['partial']['count'] ?>, <?= $dues_data['unpaid']['count'] ?>],
          backgroundColor: [green, blue, '#e74c3c'],
          borderWidth: 2
        }]
      },
      options: {
    ...
    new Chart(document.getElementById('duesAmountChart'), {
      ...
      data: {
        ...
        datasets: [{
            label: 'Amount Due',
            data: duesDueVals,
            backgroundColor: 'rgba(192,57,107,0.25)',
            borderRadius: 5
          },
          {
