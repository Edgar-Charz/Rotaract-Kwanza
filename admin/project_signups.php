...
<div class="split-layout rsvps-grid">
  ...
  <div class="card">
    ...
    <div class="event-filter-list">
      <a href="?" class="event-filter-link <?= !$project_id ? 'active' : '' ?>">All Sign-Ups</a>
      <?php foreach ($projects as $p): $c = $counts[$p['id']] ?? null; ?>
        <a href="?project=<?= $p['id'] ?>"
          class="event-filter-link--row <?= $project_id === $p['id'] ? 'active' : '' ?>">
          <span class="truncate"><?= h($p['title']) ?></span>
          <?php if ($c): ?>
            <span class="event-filter-count"><?= $c ?></span>
          <?php endif; ?>
        </a>
      <?php endforeach; ?>
    </div>
    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="flex align-center gap-2">
          <span class="text-muted fs-13"><?= count($signups) ?> sign-up<?= count($signups) !== 1 ? 's' : '' ?></span>
          ...
          <tbody>
            <?php foreach ($signups as $s): ?>
              <tr>
                ...
                <?php if (!$project_id): ?><td class="text-muted fs-12"><?= h($s['project_title']) ?></td><?php endif; ?>
                <td class="fw-bold"><?= h($s['name']) ?></td>
                <td><?= h($s['email']) ?></td>
                <td class="text-muted"><?= h($s['phone'] ?? '—') ?></td>
                <td class="text-muted text-clamp-180"><?= h($s['notes'] ?? '—') ?></td>
                ...
                <td>
                  <div class="table-actions">
                    <?php if (has_role('editor')): ?>
                      <button class="btn btn-icon btn-sm btn-info" title="Edit" aria-label="Edit" onclick="openEditSignupModal(<?= h(json_encode($s)) ?>)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="modal-dialog modal-content modal-460">
      ...
      <form method="POST">
        <div class="modal-body">
          ...
          <div class="form-group"><label>Notes</label><textarea name="notes" id="es_notes" class="mh-70"></textarea></div>
        </div>
        <div class="modal-footer">
