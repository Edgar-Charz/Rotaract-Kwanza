...
<div class="split-layout messages-grid" style="grid-template-columns:<?= $view_msg ? '1fr 1.2fr' : '1fr' ?>">

  <div class="card">
    <div class="card-header gap-8px">
      ...
      <table id="dt-messages">
        ...
        <tbody>
          <?php if ($messages): foreach ($messages as $msg): ?>
              <tr class="<?= ($view_msg && $view_msg['id'] == $msg['id']) ? 'row-selected' : '' ?>">
                <?php if (has_role('editor')): ?><td><input type="checkbox" class="row-check" value="<?= $msg['id'] ?>" onchange="updateMsgBulkBar()"></td><?php endif; ?>
                <td>
                  <div class="fw-bold nowrap"><?= h($msg['full_name']) ?></div>
                  <div class="text-muted fs-11-5"><?= h($msg['email']) ?></div>
                </td>
                <td class="text-clamp-200">
                  <?= h($msg['subject'] ?? '(no subject)') ?>
                </td>
                <td><span class="badge badge-<?= h($msg['status']) ?>"><?= h($msg['status']) ?></span></td>
                <td class="text-muted nowrap"><?= $msg['created_at'] ? date('d M Y', strtotime($msg['created_at'])) : '—' ?></td>
                <td>
                  <div class="table-actions">
                    <a href="?view=<?= $msg['id'] ?>" class="btn btn-icon btn-sm btn-info" title="View" aria-label="View"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
                        <circle cx="12" cy="12" r="3" />
                      </svg></a>
                    <?php if (has_role('editor')): ?>
                      <form id="del-msg-<?= $msg['id'] ?>" method="POST" class="d-inline">
                        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                        <input type="hidden" name="action" value="delete">
                        <input type="hidden" name="id" value="<?= $msg['id'] ?>">
                      </form>
                      <button class="btn btn-icon btn-sm btn-danger" title="Delete" aria-label="Delete" onclick="confirmDelete('del-msg-<?= $msg['id'] ?>')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  <?php if ($view_msg): ?>
    <div class="card sticky-top-80">
      ...
      <div class="card-body">
        ...
        <div class="mb-20px">
          <div class="detail-label-lg">MESSAGE</div>
          <div class="message-full"><?= h($view_msg['message']) ?></div>
        </div>

        <div class="action-row-mb20">
        ...
        <?php if (has_role('editor')): ?>
          <form method="POST">
            ...
            <div class="form-group">
              ...
              <textarea name="admin_notes" class="mh-80"><?= h($view_msg['admin_notes'] ?? '') ?></textarea>
            </div>
