      ...
      <tbody>
        <?php if ($accounts): foreach ($accounts as $a): ?>
            <tr>
              ...
              <td class="text-muted text-clamp-280"><?= h(str_replace("\n", ' · ', $a['details'])) ?></td>
              ...
              <td>
                <div class="table-actions">
                  <?php if (has_role('editor')): ?>
                    <button class="btn btn-icon btn-sm btn-info" title="Edit" aria-label="Edit" onclick="openEditAccountModal(<?= h(json_encode($a)) ?>)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M17 3a2.85 2.86 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" />
                      </svg></button>
                    <form id="del-a-<?= $a['id'] ?>" method="POST" class="d-inline">
                      <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                      <input type="hidden" name="action" value="delete">
                      <input type="hidden" name="id" value="<?= $a['id'] ?>">
                    </form>
                    <button class="btn btn-icon btn-sm btn-danger" title="Delete" aria-label="Delete" onclick="confirmDelete('del-a-<?= $a['id'] ?>')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="modal-body">
        ...
        <div class="form-row">
          ...
          <div class="form-group form-group-row pt-22px">
            <input type="checkbox" name="is_active" id="aa_active" checked class="w-auto">
            <label for="aa_active" class="fw-normal">Visible on site</label>
          </div>
        </div>
      </div>
    ...
    <form method="POST">
      <div class="modal-body">
        ...
        <div class="form-row">
          ...
          <div class="form-group form-group-row pt-22px">
            <input type="checkbox" name="is_active" id="ea_active" class="w-auto">
            <label for="ea_active" class="fw-normal">Visible on site</label>
          </div>
        </div>
      </div>
      <div class="modal-footer">
