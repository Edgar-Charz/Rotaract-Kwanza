    ...
    <table id="dt-values">
      ...
      <tbody>
        <?php if ($values): foreach ($values as $v): ?>
            <tr>
              <td>
                <div class="icon-32-primary"><?= icon_svg($v['icon_key']) ?></div>
              </td>
              <td class="fw-bold"><?= h($v['title']) ?></td>
              <td class="text-muted text-clamp-320"><?= h($v['description'] ?? '') ?></td>
              ...
              <td>
                <div class="table-actions">
                  <?php if (has_role('editor')): ?>
                    <button class="btn btn-icon btn-sm btn-info" title="Edit" aria-label="Edit" onclick="openEditModal(<?= h(json_encode($v)) ?>)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M17 3a2.85 2.86 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" />
                      </svg></button>
                    <form id="del-v-<?= $v['id'] ?>" method="POST" class="d-inline">
                      <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                      <input type="hidden" name="action" value="delete">
                      <input type="hidden" name="id" value="<?= $v['id'] ?>">
                    </form>
                    <button class="btn btn-icon btn-sm btn-danger" title="Delete" aria-label="Delete" onclick="confirmDelete('del-v-<?= $v['id'] ?>')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="modal-body">
        ...
        <div class="form-row">
          ...
          <div class="form-group form-group-row pt-22px">
            <input type="checkbox" name="is_active" id="av_active" checked class="w-auto">
            <label for="av_active" class="fw-normal">Visible on site</label>
          </div>
        </div>
      </div>
    ...
    <form method="POST">
      <div class="modal-body">
        ...
        <div class="form-row">
          ...
          <div class="form-group form-group-row pt-22px">
            <input type="checkbox" name="is_active" id="ev_active" class="w-auto">
            <label for="ev_active" class="fw-normal">Visible on site</label>
          </div>
        </div>
      </div>
      <div class="modal-footer">
