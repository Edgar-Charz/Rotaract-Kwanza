    ...
    try {
      ...
      if ($rsvp->alreadyRegistered($id, $email)) {
        ...
        try {
          ...
          Mailer::fromSettings($conn)->rsvpConfirmation($email, $name, $event);
        } catch (Throwable $e) {
        }
        ...
        <form method="POST" class="rsvp-form">
          ...
          <div class="form-row">
            <div class="form-group"><label>Full Name *</label><input type="text" name="name"
                value="<?= e($_POST['name'] ?? '') ?>" placeholder="Your name" required></div>
            <div class="form-group"><label>Email *</label><input type="email" name="email"
                value="<?= e($_POST['email'] ?? '') ?>" placeholder="your@email.com" required></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>Phone</label><input type="tel" name="phone"
                value="<?= e($_POST['phone'] ?? '') ?>" placeholder="+244 900 000 000"></div>
          ...
          <div class="form-group"><label>Notes / Dietary requirements</label><textarea name="notes" placeholder="Optional"
              rows="3"><?= e($_POST['notes'] ?? '') ?></textarea></div>
